(() => {
	...
	window.fetch = async (...args) => {
		...
		// Detect generation start (completion requests). The request body carries
		// the model id, which the content script uses for auto context-limit.
		if (url && opts.method === 'POST' && (url.includes('/completion') || url.includes('/retry_completion'))) {
			let model = null;
			try {
				if (typeof opts.body === 'string') model = JSON.parse(opts.body)?.model || null;
			} catch {
				// body not JSON; model stays unknown
			}
			post('cc:generation_start', { model });
		}
	...
	async function handleEventStream(response) {
		try {
			...
			// Stream finished: the assistant's turn is complete.
			post('cc:generation_end', { clean: sawStop });
		} catch {
			// best-effort; don't break claude.ai
			post('cc:generation_end', { clean: false });
		}
	}
